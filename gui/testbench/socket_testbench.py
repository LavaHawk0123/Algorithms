...
class Connect_Socket:
    ...
    def Testbench(self):
#        self.s1.connect((self.IP, self.port4))
#        self.s1.listen(5)
#        self.th_send_data_function.start()
        while not rospy.is_shutdown():
            print("connection established")
            time.sleep(1)
            self.colorpal_r_lambda = rospy.get_param("/colorpal/lambda/g")
            self.colorpal_g_lambda = rospy.get_param("/colorpal/lambda/r")
            self.colorpal_b_lambda = rospy.get_param("/colorpal/lambda/b")
            self.colorpal_r = rospy.get_param("/colorpal/r")
            self.colorpal_g = rospy.get_param("/colorpal/g")
            self.colorpal_b = rospy.get_param("/colorpal/b")
            ...
            self.msg = ""+str(self.colorpal_r_lambda)+","+str(self.colorpal_b_lambda)+","+str(self.colorpal_g_lambda)+","+str(self.hyd_moisture) + ","+str(self.hyd_cond)+","+str(self.hyd_temp)+","+str(self.methane_ppm) + ","+str(self.spectral)+","+str(self.ozone_ppb)+","+str(self.voc_ppm)
    ...
    def Recv_Data(self):
        ...
        c.connect((socket.gethostname(), self.port4))
        while True:
