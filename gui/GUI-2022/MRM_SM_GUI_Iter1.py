...
class Ui_MainWindow(object):
    ...
    def declare_vars(self):
        ...
        self.th_setCameraImageLabel = threading.Thread(
            target=self.getMicroscopeImage)
        ...
        self.hyd_cond_msg = "Conductivity (SU): "
        self.hyd_temp_msg = "Temperature   (C): "
        self.methane_ppm_msg = "Methane(ppm) : "
    ...
    def Recv_Data(self):
        ...
        while True:
            ...
            self.hyd_moisture = self.L[3]
            self.hyd_cond = self.L[4]
            self.hyd_temp = self.L[5]
            self.methane_ppm = self.L[6]
            self.spectral = self.L[7]
            self.ozone_ppb = self.L[8]
            self.voc_ppm = self.L[9]
            self.hyd_moisture_msg = "Soil Moisture (%): " + self.L[3]
            self.hyd_cond_msg = "Conductivity (SU): " + self.L[4]
            self.hyd_temp_msg = "Temperature   (C): " + self.L[5]
            self.methane_ppm_msg = "Methane(ppm) : " + self.L[6]
            self.spectral_msg = "Spectral Triad : " + self.L[7]
            self.ozone_ppb_msg = "Ozone (ppb) : " + self.L[8]
            self.voc_ppm_msg = "VOC Sensor (ppm) : " + self.voc_ppm
    ...
    def mousePressEvent(self, event):
        if event.button() == Qt.LeftButton:
            ...
            if(self.cameraLabel_x < 1 and self.cameraLabel_y < 1):
                ...
                self.css = "background-color : rgb("+str(self.cv_img[self.cameraLabel_x][self.cameraLabel_y][0])+","+str(self.cv_img[self.cameraLabel_x][self.cameraLabel_y][1])+","+str(self.cv_img[self.cameraLabel_x][self.cameraLabel_y][2])+");"
            print("\n css : " + self.css)
    ...
    def SetLabelValues(self):
        while True:
            ...
            self.label_10.setText(self.hyd_cond_msg)
    ...
    def getMicroscopeImageSocket(self):
        ...
        sock.bind((self.IP_add, self.port3))
        ...
        while True:
            ...
            ret, self.cv_img = cap.read()
            qt_img = self.convert_cv_qt(self.cv_img, 791, 571)
            self.main_camera.setPixmap(qt_img)
        print("goodbye")
